...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user is None:
        ...
        await state.update_data(first_step="vpn", user_id=message.from_user.id, username=message.from_user.username)
    else:
...
async def process_first_vpn(callback: CallbackQuery, state: FSMContext):
    ...
    await db.set_vpn_notify(user_id, vpn_notify)
...
async def process_first_proxy(callback: CallbackQuery, state: FSMContext):
    ...
    await db.set_proxy_notify(user_id, proxy_notify)
...
async def process_check_sub(callback: CallbackQuery, state: FSMContext):
    ...
    if is_subscribed:
        ...
        if user is None:
            ...
            await state.set_state(UserStates.waiting_first_time)
            await state.update_data(first_step="vpn", user_id=callback.from_user.id,
...
async def process_vpn(callback: CallbackQuery):
    ...
    await safe_edit_text(callback.message, "🔑 Выберите VPN-ключ:",
                         reply_markup=get_vpn_page_keyboard(keys, total, page=1))
    await callback.answer()
...
async def process_vpn_page(callback: CallbackQuery):
    ...
    await safe_edit_text(callback.message, "🔑 Выберите VPN-ключ:",
                         reply_markup=get_vpn_page_keyboard(keys, total, page=page))
    await callback.answer()
...
async def process_proxy(callback: CallbackQuery):
    ...
    await safe_edit_text(callback.message, "🛡️ Выберите прокси (нажмите для подключения):",
                         reply_markup=get_proxy_page_keyboard(proxies, total, page=1))
    await callback.answer()
...
async def process_proxy_page(callback: CallbackQuery):
    ...
    await safe_edit_text(callback.message, "🛡️ Выберите прокси:",
                         reply_markup=get_proxy_page_keyboard(proxies, total, page=page))
    await callback.answer()
...
async def process_settings(callback: CallbackQuery):
    ...
    vpn_notify = user['vpn_notify'] if user else False
    proxy_notify = user['proxy_notify'] if user else False

    await safe_edit_text(callback.message, "⚙️ Настройки уведомлений:",
                         reply_markup=get_settings_keyboard(vpn_notify, proxy_notify))
    await callback.answer()
...
async def toggle_vpn_notify(callback: CallbackQuery):
    ...
    new_value = not user['vpn_notify']
    await db.set_vpn_notify(callback.from_user.id, new_value)

    user = await db.get_user(callback.from_user.id)
    await safe_edit_text(callback.message, "⚙️ Настройки уведомлений:",
                         reply_markup=get_settings_keyboard(user['vpn_notify'], user['proxy_notify']))
...
async def toggle_proxy_notify(callback: CallbackQuery):
    ...
    new_value = not user['proxy_notify']
...
async def admin_add_vpn_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа")
        return
    await safe_edit_text(callback.message, "📅 Введите дату ключа (например: 18.06.2026):",
                         reply_markup=get_back_keyboard("admin_vpn_menu"))
...
async def admin_add_vpn_key(message: Message, state: FSMContext):
    ...
    # Спрашиваем про рассылку
    await message.answer(
        f"✅ VPN-ключ от {vpn_date} добавлен!\n\n📢 Оповестить пользователей о новом ключе?",
...
async def admin_add_proxy_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа")
        return
    await safe_edit_text(callback.message, "🛡️ Введите название прокси (например: Германия):",
                         reply_markup=get_back_keyboard("admin_proxy_menu"))
...
async def admin_add_proxy_name(message: Message, state: FSMContext):
    ...
    await message.answer("🔗 Теперь отправьте ссылку на прокси (tg://proxy или https://):",
                         reply_markup=get_back_keyboard("admin_proxy_menu"))
...
async def admin_add_proxy_url(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Прокси \"{proxy_name}\" добавлен!\n\n📢 Оповестить пользователей о новом прокси?",
...
async def confirm_broadcast(callback: CallbackQuery):
    ...
    broadcast_type = callback.data.split("_")[-1]  # vpn или proxy
...
async def admin_remove_vpn_start(callback: CallbackQuery, state: FSMContext):
    ...
    keys = await db.get_all_vpn_keys()
    if not keys:
        await safe_edit_text(callback.message, "❌ Нет ключей для удаления.",
                             reply_markup=get_back_keyboard("admin_vpn_menu"))
        await callback.answer()
        return

    text = "📋 Выберите ID ключа для удаления:\n\n" + "\n".join([f"ID {k['id']}: {k['date']}" for k in keys])
...
async def admin_remove_proxy_start(callback: CallbackQuery, state: FSMContext):
    ...
    proxies = await db.get_all_proxies()
    if not proxies:
        await safe_edit_text(callback.message, "❌ Нет прокси для удаления.",
                             reply_markup=get_back_keyboard("admin_proxy_menu"))
        await callback.answer()
        return

    text = "📋 Выберите ID прокси для удаления:\n\n" + "\n".join([f"ID {p['id']}: {p['name']}" for p in proxies])
...
async def admin_remove_sponsor_start(callback: CallbackQuery, state: FSMContext):
    ...
    sponsors = await db.get_sponsors()
    if not sponsors:
        await safe_edit_text(callback.message, "❌ Нет спонсоров.",
                             reply_markup=get_back_keyboard("admin_sponsors_menu"))
        await callback.answer()
        return
...
async def admin_blacklist_add_start(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id not in ADMIN_IDS:
        await callback.answer("⛔ Нет доступа")
        return
    await safe_edit_text(callback.message, "🚫 Введите ID пользователя для добавления в ЧС:",
...
async def admin_blacklist_remove_start(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback.message, "🚫 Введите ID пользователя для удаления из ЧС:",
                         reply_markup=get_back_keyboard("admin_blacklist_menu"))
...
async def admin_blacklist_remove_finish(message: Message, state: FSMContext):
    try:
        ...
        await message.answer(f"✅ Пользователь {user_id} удалён из ЧС!",
                             reply_markup=get_back_keyboard("admin_blacklist_menu"))
    except ValueError:
        await message.answer("❌ Введите корректный ID.", reply_markup=get_back_keyboard("admin_blacklist_menu"))
    await state.clear()
...
async def admin_support_reply_start(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback.message, "📝 Введите номер обращения для ответа:",
                         reply_markup=get_back_keyboard("admin_support_menu"))
...
async def admin_support_reply_send(message: Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer(f"✅ Ответ отправлен пользователю @{data['reply_username']}!",
                             reply_markup=get_back_keyboard("admin_support_menu"))
    except Exception as e:
...
async def admin_ad_reply_start(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback.message, "📝 Введите номер заявки для ответа:",
                         reply_markup=get_back_keyboard("admin_support_menu"))
...
async def admin_ad_reply_send(message: Message, state: FSMContext):
    data = await state.get_data()
    try:
        await message.bot.send_message(chat_id=data['reply_user_id'],
...
async def admin_broadcast_start(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback.message, f"📨 Введите сообщение для ручной рассылки ({count} чел.):",
                         reply_markup=get_back_keyboard("back_to_admin"))
...
async def admin_stats(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback.message,
                         ...
                         f"🆘 Обращений в поддержку: {stats['support_count']} (не отвечено: {stats['support_unreplied']})\n"
                         f"📢 Заявок на рекламу: {stats['ad_count']} (не отвечено: {stats['ad_unreplied']})\n\n"
