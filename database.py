...
# Общий клиент для переиспользования
client = None
...
async def get_users_with_vpn_notify():
    ...
    return [row['user_id'] for row in r.json()] if r.json() else []
...
async def get_vpn_keys_page(page: int = 1):
    ...
    content_range = r.headers.get("content-range", "0/0")
    total = int(content_range.split("/")[-1]) if "/" in content_range else 0
    return r.json() if r.json() else [], total
...
async def get_sponsors():
    ...
    return [row['channel_username'] for row in r.json()] if r.json() else []
...
async def get_stats():
    ...
    vpn_count = await count_table("vpn_keys")
